...
Interfejs serwera składa się komunikatów własnych oraz otrzymywanych od poszczególnych modułów wyświetlanych w oknie konsoli. Komunikaty są dodatkowo oznaczone kolorami odpowiadającymi poszczególnym modułom, bądź ostrzegającymi o błędach, jak pokazano na rysunkach \ref{gui_server1}, \ref{gui_server2} i \ref{gui_server3}.
...
Graficzny interfejs użytkownika właściwego widoku wizualizacji składa się z widoku jednej z kamer do wyboru oraz kokpitu. Kokpit w lewym dolnym rogu zawiera radar pozwalający na wykrycie obcych BSP w okolicy, w prawym dolnym rogu znajduje się schemat silników BSP gdzie możemy odczytać ich prędkość obrotu, a w prawym górnym rogu jest pokazany stan wyposażenia kontrolowanego BSP. Ponadto, w kokpicie na dole ekranu znajduje się sztuczny horyzont, którego wizualizacja zgodna jest z powszechnie stosowaną konstrukcją. Pozwala on na uzyskanie informacji o miejscu BSP w przestrzeni, jego orientacji i prędkościach. Ponadto, w lewym górnym rogu sztucznego horyzontu znajduje się obecny tryb lotu BSP, a parametry przez ten tryb narzucane są narysowane na pozostałych fragmentach sztucznego horyzontu. Użytkownik może również włączyć tryb mapy, aby zobaczyć swoje położenia w świecie symulacji.
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=1\textwidth]{game_view_observer.png}
	\caption{Interfejs graficzny wizualizacji w widoku obserwatora.}
	\label{gui_game4}
\end{figure}
...
Zasoby zawierają modele i grafiki niezbędne do pracy wizualizacji. Wersja zasobów to pierwsze 8 znaków numeru checksum, który jest generowany na podstawie zawartości assetów i przekazywany wizualizacji przez serwer, pozwalając użytkownikowi pobrać brakujące zasoby w razie gdy ich nie posiada. Pobrana zawartość jest umieszczana w katalogu nazwanego od wersji zasobów. Zawiera on następujące katalogi:
...
\subsubsection*{Data}
\begin{itemize}
\item "available\_control\_modes.yaml" -- Plik konfiguracyjny zawierający dozwolone tryby kontroli lotu. Przykładowy plik konfiguracyjny został załączony jako "available\_control\_modes\_template.yaml".
\end{itemize}
\subsubsection*{Drones}
\begin{itemize}
\item \textbf{\textbf{\textit{NazwaModeluBSP}}} -- Katalogi zawierające modele wraz z teksturami dla konkretnego modelu BSP w folderach \textbf{model} i \textbf{textures}.
\end{itemize}
\subsubsection*{Core}
\begin{itemize}
	\item \textbf{\textbf{\textit{NazwaMapy}}} -- Katalogi zawierające modele oraz tekstury mapy w folderach \textbf{model} i \textbf{textures}. W folderze \textbf{model} dodatkowo znajduje się obraz mapy z lotu ptaka pod nazwą "minimap.png".
\end{itemize}

W katalogu \textbf{model} domyślnie znajduje się model obiektu w postaci pliku z rozszerzeniem GLTF, a więc "model.gltf" wraz z odpowiadającym mu "model.bin" oraz pliku z rozszerzeniem OBJ: "model.obj". \\


Gdy wizualizacja wykryje brakujące tekstury, zastępowane są one przez grafikę brakującej tekstury "missing.jpg" znajdującą się w katalogu \textbf{assets}.\\
